...
and nt_paired_comment str = 
  let comment = caten (caten (word "{") (star nt_sexpr)) (word "}") in
...
and string_meta_char str = 
  let meta = disj_list [
    (* pack (word "\"") (fun _-> char_of_int(34)); *)
                        ...
                        pack (word "~~") (fun _-> char_of_int(126));
                        pack (word "\\f") (fun _-> char_of_int(12))] in
  meta str
...
and string_hex_char str = 
  ...
  let r = plus (disj_list [range '0' '9';range 'a' 'f'])  in
  let r = caten prefix r in
  let r = pack r (fun ((_,x),h) -> char_of_int(int_of_string(list_to_string(['0']@x@h)))) in
  r str
...
and string_char str =
  let state = disj_list [string_meta_char;string_literal_char;string_hex_char] in
  state str

and nt_string str =
  let qot = char '\"' in
  let normal_string = pack (star string_char) (fun lst -> List.fold_left (fun hd tl -> hd@tl) lst []) in
  let normal_string = pack normal_string (fun s -> ScmString(list_to_string(s))) in
  let interpo = string_interpolated in 
  (* first case: only string ,note:dealing with the "\"" *)
  let nt1  = pack (caten normal_string qot) (fun (expr,_) -> expr) in
  (*end of the first case*)
  (* second case: only interpolated *)
  let nt2 = pack (caten interpo qot) (fun (expr,_) -> expr) in
  (* end of second case *)
  (* third case: complicated string *)
  let nt3 = disj normal_string interpo in
  let nt3 = caten nt3 (plus nt3) in
  let nt3 = caten nt3 qot in
  let nt3 = pack nt3 (fun ((expr,exprs),_) -> List.fold_right (fun hd tl -> ScmPair(hd,tl))
                                              [ScmSymbol ("string-append");ScmString("~a");sexpr] ScmNil) in
  (* end of third case *)
  let nt = disj_list[nt1;nt2;nt3] in
  let nt = pack (caten qot nt) (fun (_,s) -> s) in
  nt str
